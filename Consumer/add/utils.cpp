...
void NdnRtcUtils::createLibFace(/*const new_api::GeneralParams& generalParams*/)
{
        ...
        (LibraryFace.get() && LibraryFace->getTransport()->getIsConnected() == false))
    {
		...
		KeyChain key();
		boost::shared_ptr<KeyChain> defaultKeyChain ( key );
        LibraryFace = FaceProcessor::createFaceProcessor(
						"localhost"/*generalParams.host_*/,
						45216/*generalParams.portNum_*/, 
						defaultKeyChain/*NdnRtcNamespace::defaultKeyChain()*/);
